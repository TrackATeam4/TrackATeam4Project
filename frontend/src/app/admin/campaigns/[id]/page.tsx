  ...
  useEffect(() => {
    const fetchCampaign = async () => {
      ...
      try {
        ...
        if (Array.isArray(res.data?.signups)) {
          (c as CampaignDetail).signups = res.data.signups;
        }
    ...
    <div className="px-6 py-8 text-slate-700">
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          {
            label: "Time",
            value: campaign.start_time && campaign.end_time ? `${campaign.start_time} - ${campaign.end_time}` : "-",
          },
      ...
      <div className="mt-8 rounded-2xl border border-yellow-100 bg-white p-6 shadow-lg shadow-yellow-100/60">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-semibold text-[#0F172A]">Signups and Attendance</h2>
        ...
        {signups.length === 0 ? (
          <p className="py-8 text-center text-sm text-slate-400">No signups yet. Promote this campaign to start volunteer turnout.</p>
        ) : (
              ...
              <tbody>
                {signups.map((s, i) => (
                  <tr key={s.user_id || i} className="border-b border-yellow-50 last:border-0">
                    ...
                    <td className="py-3 pr-4 text-slate-500">{s.status || "-"}</td>
                    <td className="py-3">
                      {s.attended ? (
