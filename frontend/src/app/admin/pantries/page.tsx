...
export default function AdminPantriesPage() {
  ...
  const handleVerify = async (id: string) => {
    ...
    try {
      ...
      setNotice("Pantry verified successfully.");
    } catch (err) {
  ...
  const formatHours = (hours: Pantry["hours"]) => {
    if (!hours) return "-";
    if (typeof hours === "string") return hours;
    const preview = Object.entries(hours)
      .slice(0, 2)
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-yellow-100 bg-white shadow-lg shadow-yellow-100/60">
        {loading ? (
          ...
          <p className="py-16 text-center text-sm text-slate-400">
            No food pantries found. Add or import pantry listings to unlock verification workflow.
          </p>
        ) : (
              ...
              <tbody>
                {pantries.map((p) => (
                  <tr key={p.id} className="border-b border-yellow-50 last:border-0 transition hover:bg-[#FFFEF5]">
                    ...
                    <td className="px-5 py-4 text-slate-500">{formatHours(p.hours)}</td>
