...
export default function AdminFlyersPage() {
  ...
  const inputCls =
    "w-full rounded-xl border border-yellow-100 bg-white px-4 py-2.5 text-sm text-slate-700 outline-none transition focus:border-[#F5C542] focus:ring-2 focus:ring-[#F5C542]/30 placeholder:text-slate-400";
    ...
    <div className="px-6 py-8 text-slate-700">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        {formMode === "idle" && (
          <button
            onClick={openCreate}
            className="rounded-xl bg-[#F5C542] px-5 py-2.5 text-sm font-semibold text-[#1A1A1A] shadow-lg shadow-[#F5C542]/30 transition hover:bg-[#E0B63A]"
          >
            + New Template
          </button>
        )}
          ...
          <div className="mt-4 space-y-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="rounded-xl bg-[#F5C542] px-5 py-2.5 text-sm font-semibold text-[#1A1A1A] shadow-lg shadow-[#F5C542]/30 transition hover:bg-[#E0B63A] disabled:opacity-50"
              >
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-yellow-100 bg-white shadow-lg shadow-yellow-100/60">
        {loading ? (
          <div className="flex items-center justify-center py-16">
            <div className="h-8 w-8 animate-spin rounded-full border-4 border-[#F5C542]/40 border-t-[#E0B63A]" />
          </div>
        ) : templates.length === 0 ? (
          <p className="py-16 text-center text-sm text-slate-400">No flyer templates yet.</p>
        ) : (
                ...
                {templates.map((t) => (
                  <tr key={t.id} className="border-b border-yellow-50 last:border-0 transition hover:bg-[#FFFEF5]">
                    ...
                    <td className="px-5 py-4">
                      {t.is_active !== false ? (
                        <span className="inline-flex rounded-full bg-[#F5C542]/15 px-2.5 py-1 text-xs font-semibold text-[#A66F00]">Active</span>
                      ) : (
