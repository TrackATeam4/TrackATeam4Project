...
export default function AdminLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      try {
        const res = await authFetch<{ id?: string; role?: string }>("/auth/me");
        ...
        await signOut();
        setStatus("error");
        setMessage("Could not verify admin privileges. Please try again.");
        return;
      }
      ...
      >
        <div className="rounded-3xl border border-yellow-100 bg-white p-8 shadow-2xl shadow-yellow-200/50 sm:p-10">
          <div className="mb-6 text-center">
            <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-emerald-100 text-xl">A</div>
            <h1 className="text-xl font-bold text-[#0F172A]">Admin Console</h1>
            <p className="mt-1 text-sm text-slate-500">Manage campaigns, volunteers and analytics</p>
          </div>
