...
@router.post("/events/{event_id}/impact", status_code=201)
def create_event_impact_report(
    event_id: str = Path(..., pattern=_UUID_PATTERN),
    body: ImpactReportCreate = ...,
    user=Depends(get_current_user),
    supabase=Depends(get_supabase_client),
):
    """Submit a post-event impact report for a post-based event. Author only."""
    user_id = user.user.id
    ...
    if event.get("author_id") != user_id:
        raise HTTPException(status_code=403, detail="Only the event author can submit a report")
    ...
    try:
        ...
        insert_result = supabase.table("event_impact_reports").insert(report_row).execute()
        report_data = insert_result.data
    except Exception as exc:
        if "unique" in str(exc).lower() or "duplicate" in str(exc).lower():
            raise HTTPException(
                status_code=409,
                detail="Impact report already submitted for this event",
            )
        logger.error("Failed to insert impact report for event %s: %s", event_id, exc)
        raise HTTPException(status_code=500, detail="Failed to create report")
...
def get_event_impact_report(
    event_id: str = Path(..., pattern=_UUID_PATTERN),
    user=Depends(get_current_user),
    supabase=Depends(get_supabase_client),
):
    """Get the impact report for a post-based event. Auth required."""
    report_result = (
        supabase.table("event_impact_reports")
        .select("*")
